...
- **Bug reports & feature requests** — please [open an issue](https://github.com/autousers-ai/cli/issues/new). The team monitors issues here directly, triages them, and applies fixes upstream. Resolved fixes propagate back to this mirror automatically on the next release.
- **Questions & feedback** — same place: [issues](https://github.com/autousers-ai/cli/issues). Use the `question` label or just the regular form.
...
Every release force-pushes a single fresh commit to this repo's `main` branch — that commit represents the exact source tree of the latest published npm package. Tags (`cli-v*`) trigger automated publication to npm with [verifiable provenance](https://docs.npmjs.com/generating-provenance-statements), attesting that the published tarball was built from the commit you can browse here. The provenance signature is verifiable via `npm audit signatures` or directly on the [npm package page](https://www.npmjs.com/package/@autousers/cli).

## Why is this repo a mirror

Real development happens upstream against the broader Autousers monorepo (web app, server, schema, MCP, CLI, etc. — they share types and migrations). Splitting the CLI out as a public, single-commit mirror gives users a verifiable source-to-tarball chain without requiring the team to maintain a separate development repo.
