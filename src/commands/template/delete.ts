/**
 * `autousers template delete <id>` — soft-delete a template.
 * Confirms unless `--yes`; non-TTY requires `--yes`.
 */
...
export async function templateDeleteAction(
  id: string,
  opts: TemplateDeleteOpts,
  cmd: Command
): Promise<void> {
  ...
  try {
    ...
    if (!opts.yes) {
      process.stdout.write(
        `${red("⚠")} Delete template ${id}? This cannot be undone. [y/N] `
      );
    ...
    await ctx.client.delete(`/api/v1/templates/${encodeURIComponent(id)}`);
...
export function registerTemplateDeleteCommand(parent: Command): Command {
  return parent
    .command("delete <id>")
