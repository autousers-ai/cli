/**
 * Tests for `autousers eval share <id> --email --role`.
 */
...
describe("eval share", () => {
  it("POSTs /shares with email + default role viewer", async () => {
    const post = vi.fn().mockResolvedValueOnce({
      data: { id: "s1", email: "u@x.com", role: "viewer" },
    });
    ...
    expect(path).toBe("/api/v1/evaluations/e1/shares");
    expect(body).toEqual({ email: "u@x.com", role: "viewer" });
    expect(cap.stdout).toContain("Shared evaluation");
  });

  it("accepts --role editor and emits JSON envelope in --json mode", async () => {
    const post = vi.fn().mockResolvedValueOnce({
      data: { id: "s2", email: "u@x.com", role: "editor" },
    });
    createClient.mockResolvedValueOnce({ post });
    const cap = capture();
  ...
  it("exits 4 when --role is not in the enum", async () => {
    const post = vi.fn();
    createClient.mockResolvedValueOnce({ post });
    const cap = capture();
    ...
    expect(cap.stderr).toContain("--role must be one of");
  });
