      ...
      <Box gap={2}>
        ...
        {data.overall.totalRatings > 0 && (
          <Text>
            ...
            {/* The count above is the USABLE one — the server has already
                held out every rating that is not a judgement. Printing it
                alone read as "you have 8 ratings" on an evaluation launched
                with ten, and the two blocked scrapes behind the gap are the
                one fact that tells someone to enable the proxy. The Stats tab
                carries the reasons and the remedy; this is the pointer to
                them, because the header is on screen whatever tab is open. */}
            {data.overall.excluded.total > 0 && (
              <Text color="yellow">
                {" "}
                (+{data.overall.excluded.total} excluded — see Stats)
              </Text>
            )}
          </Text>
        )}
