...
// state update → render) under concurrent vitest load. 4500ms sits
// just under vitest's default 5s per-test budget so the poll always
// gets to assert before the test itself times out, while still giving
// the input → handler → fetch → state-update chain enough headroom
// when the full suite (~335 files, 3.4k tests) is thrashing.
...
describe("Settings", () => {
  ...
  it("`r` on a highlighted key opens the rename input and PATCHes { name } on submit", async () => {
    ...
    // Per-char tick bumped from 30 → 50ms — the InlineTextInput state
    // update can race the next keystroke under concurrent vitest load,
    // producing interleaved characters like "Local deveamed-ey".
  ...
  it("`s` on a highlighted key opens the scope input and PATCHes { scopes: [...] } on submit", async () => {
    ...
    // sole submission; backspace once per char. Per-char tick bumped
    // from 30 → 50ms so the InlineTextInput state update has time to
    // commit before the next keystroke arrives under concurrent vitest
    // load.
