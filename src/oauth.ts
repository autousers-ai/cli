...
export function openUrl(url: string): void {
  // Safety: vitest sets `VITEST=true` automatically. Skip the actual
  // browser-open spawn under tests so suites that hit this code path
  // (or import the module) don't pop a tab in the developer's browser.
  // Same reason the test in oauth.test.ts assumes a no-op.
